      ...
      <div style={{maxWidth:1000,margin:"0 auto",padding:"24px 20px 80px"}}>
        ...
        <div style={{
          ...
          <button onClick={()=>setSettingsOpen(v=>!v)} style={{
            ...
            borderBottom:settingsOpen?"1px solid #F3F4F6":"none",
          }}>
            <span style={{fontSize:14,fontWeight:700,color:"#111"}}>
              ⚙️ 設定
              {company&&<span style={{fontWeight:400,color:"#9CA3AF",fontSize:13}}> · {company}</span>}
            </span>
            <span style={{color:"#9CA3AF",fontSize:14,transform:settingsOpen?"rotate(180deg)":"none",transition:"transform .2s"}}>▼</span>
          </button>
          ...
          {settingsOpen&&(
            <div style={{padding:"20px"}}>
              ...
              <div style={{marginBottom:20}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"32px 1fr",rowGap:7,alignItems:"center"}}>
                  {DAYS_JA.map((d,dow)=>(
                    <React.Fragment key={dow}>
                      <div style={{
                        fontSize:12,fontWeight:700,textAlign:"center",
                        color:dow===0?"#F43F5E":dow===6?"#0EA5E9":"#6B7280",
                      }}>{d}</div>
                      <div style={{display:"flex",gap:6}}>
                        {POST_TYPES.map(pt=>{
                          const active=ws[dow].has(pt.id);
                          return(
                            <button key={pt.id} onClick={()=>toggleType(dow,pt.id)} style={{
                              padding:"5px 14px",borderRadius:20,cursor:"pointer",
                              border:`1.5px solid ${active?pt.color:"#E5E7EB"}`,
                              background:active?pt.lightBg:"#fff",
            ...
            <div ref={calRef} style={{background:"#fff",padding:"24px 24px 20px"}}>
              ...
              <div style={{
                ...
                <div>
                  ...
                  <div style={{fontSize:11,color:"#6B7280",marginTop:2}}>{PLANS.find(p=>p.id===planId)?.label} プラン</div>
                </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3,marginBottom:3}}>
                {DAYS_JA.map((d,i)=>(
                  <div key={i} style={{
                    ...
                    background:i===0||i===6?"#FEF2F2":"#F9FAFB",
                  ...
                  return(
                    <div key={idx}
                      ...
                      style={{
                        ...
                        background:isDrop?"#F0FDF4":isRed?"#FEF2F2":"#fff",
                        border:isDrop?"2px solid #22c55e":`1px solid ${isRed?"#FECACA":"#E5E7EB"}`,
                        height:130,
